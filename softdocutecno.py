# VERSION_VALIDADA_MODELO_NEGOCIOS_TRL_6_7_8_TEXTOS_150_220_PALABRAS_SIN_NOMBRE_PROYECTO_EN_CAMPOS
...
# Basado en formato de identificación de modelo de negocios TRL 6 / TRL 7 / TRL 8
# =====================================================
...
def limitar_caracteres_canvas(texto: str, nombre_proyecto: str = "", codigo_proyecto: str = "", min_chars: int = 150, max_chars: int = 220) -> str:
    """
    Ajusta el texto entre 150 y 220 PALABRAS, sin puntos suspensivos,
    sin menciones repetidas del nombre/código del proyecto y sin frases instructivas.
    Conserva redacción continua y corrige expresiones básicas para que el contenido
    responda directamente cada pregunta del modelo de negocio.
    """
    import re
    ...
    frases_no_deseadas = [
        ...
        "de acuerdo con la descripción", "este apartado", "este bloque", "la descripción del producto",
        "los aspectos suministrados", "la información suministrada", "el formulario", "el campo",
        "a partir de la información", "con base en la información", "el proyecto se llama"
    ]

    for frase in frases_no_deseadas:
        texto = re.sub(re.escape(frase), "", texto, flags=re.IGNORECASE)

    # Correcciones y limpieza básica de redacción.
    reemplazos = {
        "  ": " ",
        " ,": ",",
        " .": ".",
    ...
    # Eliminar posibles menciones residuales al nombre o código del proyecto por coincidencias flexibles.
    for valor in [nombre_proyecto, codigo_proyecto]:
        valor = str(valor or "").strip()
        if valor:
            texto = re.sub(re.escape(valor), "la solución", texto, flags=re.IGNORECASE)

    ampliaciones = [
        "La relación con el contexto de implementación permite orientar la propuesta hacia necesidades reales, condiciones operativas específicas y oportunidades de adopción gradual con usuarios cercanos al problema.",
        "La validación inicial debe apoyarse en demostraciones, pilotos, acompañamiento técnico, comunicación clara y seguimiento a resultados que permitan ajustar la solución antes de escalarla.",
        "El enfoque prioriza utilidad práctica, soporte oportuno, mejora continua, evidencia de funcionamiento y adaptación a las características del segmento objetivo y del territorio donde se implementa.",
        "La hipótesis de negocio se fortalece al contrastar beneficios, costos, canales, recursos y aliados con actores reales, evitando promesas no comprobadas y manteniendo trazabilidad de los aprendizajes obtenidos."
    ]

    i = 0
    while len(texto.split()) < min_chars:
        texto = (texto + " " + ampliaciones[i % len(ampliaciones)]).strip()
        i += 1

    palabras = texto.split()
    if len(palabras) > max_chars:
        texto = " ".join(palabras[:max_chars]).rstrip(" ,;:")
...
def generar_modelo_negocio_modo_prueba(
    ...
    return {
        clave: limitar_caracteres_canvas(valor, nombre_proyecto, codigo_proyecto, 150, 220)
...
Construye un informe de identificación del modelo de negocios aplicado específicamente al producto descrito por el usuario. El nivel TRL será TRL 6, TRL 7 o TRL 8 según el dato suministrado.

Reglas obligatorias:
1. Usa el nombre del proyecto solo como contexto interno. Prohibido incluirlo, repetirlo o iniciar respuestas con el nombre del proyecto dentro de los 9 campos.
...
8. Cada campo debe tener entre 150 y 220 palabras.
...
"""
    ...
    try:
        ...
        for item in ITEMS_LEAN_CANVAS:
            ...
            datos[item] = limitar_caracteres_canvas(texto_limpio, nombre_proyecto, codigo_proyecto, 150, 220)
...
def generar_pdf_lean_canvas(datos: dict) -> str:
    ...
    contenido = {
        item: limitar_caracteres_canvas(datos.get("contenido_lean_canvas", {}).get(item, ""), datos.get("nombre_proyecto", ""), datos.get("codigo_proyecto", ""), 150, 220)
        for item in ITEMS_LEAN_CANVAS
    }
